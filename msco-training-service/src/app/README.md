...
As `training-service` is quite minimal, take a look at the two provided submodules
...
angular.module( 'training-service', [
  'templates-app',
  'templates-common',
  'training-service.home',
  'training-service.about'
  'ui.router',
  'ui.route'
])
```
...
.controller( 'AppCtrl', function AppCtrl ( $scope, $location ) {
  $scope.$on('$stateChangeSuccess', function(event, toState, toParams, fromState, fromParams){
    if ( angular.isDefined( toState.data.pageTitle ) ) {
      $scope.pageTitle = toState.data.pageTitle + ' | training-service' ;
    }
  });
})
```
...
One of the design philosophies of `training-service` is that tests should exist
