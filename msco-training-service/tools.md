# The Tools Used in `training-service`

## Introduction

`training-service` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `training-service` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`training-service` uses git to manage its codebase. While in theory you don't have
to use Git once you download `training-service`, this project makes the assumption
...
While `training-service` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `training-service` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `training-service`; Grunt is
...
to get started with `training-service`. But for those curious or looking to go a
...
Grunt is the engine behind `training-service`. It's the magic that makes it move.
...
`training-service` comes with a `bower.json` file that looks something like this:
...
in `training-service` by default in there to see what I mean.
